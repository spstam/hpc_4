...
//     cudaTextureObject_t d_in1, d_luts1;
//     unsigned char *d_out1;
//     dim3 render_block(32, 32);
//     dim3 render_grid((img_in.w + 31) / 32, (img_in.h + 31) / 32);
//     render_clahe_kernel<<<render_grid, render_block>>>(d_in1, d_out1, d_luts1, img_in.w, img_in.h, grid_w, grid_h);
...
int main(int argc, char *argv[]){
    ...
    // Allocate host-side pointers for device memory
    unsigned char *d_in, *d_out;
    int *d_luts;
    size_t img_size = img_in.w * img_in.h * sizeof(unsigned char);
    size_t lut_size = grid_w * grid_h * 256 * sizeof(int);

    // Start GPU Timer Scope
    {
        GpuTimer timer;
        timer.Start();
        ...
        // 2. Copy Input Image to GPU
        cudaMemcpy(d_in, img_in.img, img_size, cudaMemcpyHostToDevice);

        // 3. Run Histogram Kernel (Generates the LUTs in global memory)
        dim3 hist_block(TILE_SIZE, 8);
        dim3 hist_grid(grid_w, grid_h);
        
        histogram_lut_kernel<<<hist_grid, hist_block>>>(d_in, d_luts, img_in.w, img_in.h, grid_w, CLIP_LIMIT);
        
        // Ensure LUTs are fully computed before binding texture
        cudaDeviceSynchronize();

        // 4. Create Texture Object for LUTs
        // --------------------------------------------------------------------
        ...
        // FIX 1: Correct Channel Descriptor for Integer Data
        // We use cudaCreateChannelDesc<int>() to match the 'int' type of d_luts
        resDesc.res.linear.desc = cudaCreateChannelDesc<int>();

        struct cudaTextureDesc texDesc;
        memset(&texDesc, 0, sizeof(texDesc));
        
        // FIX 2: Read Mode ElementType
        // As per PDF Page 6/8, we choose ElementType to get raw Integers.
        // If we used NormalizedFloat, we would get 0.0-1.0 floats which is wrong for LUTs.
        ...
        // dim3 render_grid((img_in.w + 31) / 32, (img_in.h + 31) / 32);
        dim3 render_block(32, 32);
        int vec_w = img_in.w / 4; 
        
        // Calculate grid dimensions based on the vectorized width
        dim3 render_grid((vec_w + 31) / 32, (img_in.h + 31) / 32);
        // FIX 3: Pass Valid Arguments
        // We pass the RAW pointer 'd_in' (not the uninitialized d_in1 texture)
        // We pass the RAW pointer 'd_out' (not uninitialized d_out1)
        // We pass the VALID texture 'tex_luts' created above
        render_clahe_kernel<<<render_grid, render_block>>>(d_in, d_out, tex_luts, img_in.w, img_in.h, grid_w, grid_h);
